...
class Fz_sim(Fz_Comfort):
    ...
    def load_var(self): 
        '''
        Método carrega variáveis aleatórias para 
        * temperatura
        * umidade
        * humor do usuário - master
        '''
        np.random.seed(SEED)
        self.t  = round(randint(0, 40)+random(),1)    
        self.u  = round(randint(0, 99)+random(),1)
        self.h  = round(randint(0, 9)+random(),1)        
        
    def Fuzificar(self,graf=0,imp_texto=0):
        ...
        return self.t, self.u, self.h, res
...
if __name__ == "__main__":
    # Fz_sim().Fuzificar(graf=1,imp_texto=1 )
    pass
