...
class SHC_ComfortFunctions():
    ...
    def CompareComfort(self,tar= 1, alfa = 0.25, carga = 1):
         ...
         Method configures the SHC of each residence using the SHC_conf.
          ...
          fuzzy and non-fuzzy comfort, and list of loads.
         '''
         alfa_sim=alfa
         for user_var_i in tqdm(self.user_var):
             ...
             for tar,i in enumerate(['TC','ToU']):
                 print(f'\n Tarifa {i}')
                 for j in tqdm(self.comfort_function):        
...
def sim_full():
    ...
    for i in alfa:
        SHC_ComfortFunctions(default=1,
                           ...
                           user_variable=([10,60,2],[30,65,5],[39,70,9])
                           )
sim_full()
